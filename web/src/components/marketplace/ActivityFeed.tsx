...
export function ActivityFeed() {
    // Mock data for now, ideally connected to Indexer via websocket or polling
    const activities = [
                    ...
                    <div key={i} className="p-4 hover:bg-white/5 transition-colors cursor-pointer group">
                        <div className="flex items-center justify-between mb-1">
                            <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                                act.type === 'SALE' ? 'bg-green-500/20 text-green-400' : 'bg-blue-500/20 text-blue-400'
                            }`}>
                                {act.type === 'SALE' ? 'SOLD' : 'LISTED'}
                            </span>
