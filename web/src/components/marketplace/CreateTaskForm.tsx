...
export function CreateTaskForm({ onTaskCreated }: CreateTaskFormProps) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (fileList.length === 0) {
                setFileStatusMsg('❌ No valid .mv files found (excluding tests/deps)!');
                return;
            }
    ...
    const handleSubmit = async () => {
        ...
        // Require bytecode for package tasks
        if (taskType === 'package' && modulesBase64.length === 0) {
            toast.error('At least one package module (.mv) is required');
            return;
        }
        ...
        try {
            ...
            await createTask(
                ...
                reward,
                lockDurationMs,
                new TextEncoder().encode(JSON.stringify(modulesBase64)), // Serialize modules list to bytes
                ...
                {taskType === 'package' && (
                    <div>
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                                <Label className="brutal-btn cursor-pointer bg-blue-600 hover:bg-blue-500 text-white text-sm py-2 px-4 rounded-lg flex items-center gap-2 transition-colors">
                                ...
                                <Tooltip content="Upload all compiled Move bytecode files (.mv) for your package. Exclude dependencies and tests.">
                                    <Info className="w-4 h-4 text-gray-500 hover:text-white cursor-help" />
                                </Tooltip>
                            </div>
                            ...
                            <div className="text-sm">
                                {modulesBase64.length > 0 ? (
                                    <span className="text-green-400 font-medium">
                                        ✅ Loaded {modulesBase64.length} module(s)
                                    </span>
                                ) : (
                                    <span className="text-gray-500">No modules loaded</span>
                                )}
                            </div>

                            {fileStatusMsg && (
                                <p className={`text-xs ${fileStatusMsg.startsWith('❌') ? 'text-red-400' : 'text-gray-400'}`}>
                                    {fileStatusMsg}
                                </p>
                            )}
                        </div>
                        <p className="text-xs text-gray-500 mt-2">
                            Select all .mv files for your package. Dependencies and tests will be automatically filtered.
                        </p>
                    </div>
                )}
